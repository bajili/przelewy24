...
<script type="text/javascript">
    ...
    function payInShopSuccess(status) {
        $.ajax('{$p24_url_status}', {
            ...
            success: function (response) {
                window.location = response.trim() == 'OK' ? '{$p24_url_return}' : '{$p24_url_cancel}';
            }
        });
    ...
    function payInShopFailure() {
        ...
        window.location = '{$p24_url_cancel}{if $get_order_id}?order_id={$get_order_id}{/if}';
    }
        ...
        {if empty($p24FreeOrder) or $p24FreeOrder!=true}
            ...
            {else}
                ...
                <form action="{$p24_url}" method="post" id="przelewy24Form" name="przelewy24Form"
                    ...
                    <input type="hidden" name="p24_ecommerce2" value="{$p24_ecommerce}"/>
                    <input type="hidden" name="p24_sign" value="{$p24_sign}"/>
                    <input type="hidden" name="p24_regulation_accept" {if $accept_in_shop_accepted}value="1"{/if}/>
                ...
                {else}
                    {if ($oneclick && empty($p24_method) && ($ccards_forget != 1)) || $extracharge_enabled}
                        <hr>
                    {/if}
                    {* jest jakiś wybór! zapisana karta, cc ajax, ostatnia metoda płatności itp *}
